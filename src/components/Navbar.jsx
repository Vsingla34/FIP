    ...
    <nav id="navbar">
      <div className="nav-inner">
            ...
            <>
              ...
              {isAdmin && (
                <Link to="/admin" className="nav-link" style={{
                  fontSize:'12px', display:'flex', gap:'5px', alignItems:'center',
        ...
        <button className="nav-hamburger"
                onClick={() => setMobileOpen(!mobileOpen)} aria-label="Open menu">
          <span/><span/><span/>
        </button>
      </div>
        ...
        {user ? (
          <>
            <Link to="/dashboard" className="nav-mobile-link">
              <i className="fa-solid fa-circle-user" style={{width:'18px',color:'var(--orange)'}}></i>
              My Dashboard
            </Link>
            {isAdmin && (
              <Link to="/admin" className="nav-mobile-link">
                <i className="fa-solid fa-shield-halved" style={{width:'18px',color:'var(--orange)'}}></i>
                Admin Panel
              </Link>
            )}
          ...
          <div style={{display:'flex',gap:'10px',marginTop:'16px',padding:'0 16px 16px'}}>
            <button className="btn btn-outline-blue" style={{flex:1,justifyContent:'center'}}
                    onClick={() => { openModal('login'); setMobileOpen(false); }}>Log In</button>
            <Link to="/membership" className="btn btn-primary"
                  style={{flex:1,justifyContent:'center'}}
                  onClick={() => setMobileOpen(false)}>Join FIP</Link>
          </div>
        )}
      </div>
    </nav>
